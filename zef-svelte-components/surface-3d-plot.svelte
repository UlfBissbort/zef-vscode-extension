...
{#if data?.__type === 'ET.Surface3dPlot'}
  <section class="surface-plot" aria-label={data.title ?? '3D surface plot'}>
    {#if data.title || data.subtitle}<header>{#if data.title}<h2>{data.title}</h2>{/if}{#if data.subtitle}<p>{data.subtitle}</p>{/if}</header>{/if}
    <div bind:this={host} class="canvas" aria-label="Drag to orbit; scroll to zoom"></div>
