...
<script>
  ...
  function toggleFullWidth() { isFullWidth = !isFullWidth; window.parent.postMessage({ type: 'zefEntityToggleFullWidth', entityType: 'Graph' }, '*'); }
  ...
  onMount(() => { const observer = new ResizeObserver(() => void tick().then(reportHeight)); observer.observe(figure); reportHeight(); return () => observer.disconnect(); });
