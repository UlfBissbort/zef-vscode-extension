...
<script>
  ...
  function modelFor(value, offset) {
    ...
    return { timeZone, reference, earliest, latest, rangeMinutes: latest - earliest, days, events, placements };
  }
...
{#if data?.__type === 'ET.Calendar'}
  <section bind:this={calendar} class:full-width={isFullWidth} class="calendar metal" style={`--hours:${model.rangeMinutes / 60}`} aria-label={data.title ?? 'Weekly calendar'}>
    ...
    <div class="cal"><div class="head"><div class="head-cell"></div>{#each model.days as day}<div class:today={day.key === todayKey} class:weekend={day.index >= 5} class="head-cell"><span class="dow">{day.weekday}<span class="num">{day.day}</span></span></div>{/each}</div><div class="body"><div class="gutter">{#each hours as hour}<span class="tick" style={`top:${(hour - model.earliest) / 60 * (isFullWidth ? 56 : 44)}px`}>{hourLabel(hour)}</span>{/each}</div>{#each model.days as day}<div class:today={day.key === todayKey} class:weekend={day.index >= 5} class="col">{#each model.events.filter(event => key(event.startMilliseconds, model.timeZone) === day.key) as event (event.__local_name ?? event.index)}<article class:compact={event.durationSeconds < 2400} class={`event ev-${event.category}`} style={eventStyle(event, model)} title={`${event.title ?? 'Untitled event'} · ${clock(event.startMilliseconds, model.timeZone)} – ${clock(event.startMilliseconds + event.durationSeconds * 1000, model.timeZone)}`}><div class="ev-time">{clock(event.startMilliseconds, model.timeZone)} – {clock(event.startMilliseconds + event.durationSeconds * 1000, model.timeZone)}</div><div class="ev-title">{event.title ?? 'Untitled event'}</div></article>{/each}</div>{/each}{#if today && weekOffset === 0 && minutes(model.reference, model.timeZone) >= model.earliest && minutes(model.reference, model.timeZone) < model.latest}<div class="now" style={`top:${(minutes(model.reference, model.timeZone) - model.earliest) / 60 * (isFullWidth ? 56 : 44)}px;--col-i:${today.index}`}></div>{/if}</div></div>
