/* +++
ET.TypeScriptFile('🍃-0fb1abbb8133f17cb56b',
  tag_=[],
  created=Time('2026-03-09 14:18:20 +0800')
)
+++ */
...
import { checkInstallation, runInstallation, installCli, ensureZefVenvDiscoverable } from './installer';
        ...
        vscode.commands.registerCommand('zef.runBlockAtCursor', async () => {
            ...
            if (!isZefDocument(editor.document)) {
                ...
                if (!treatAllMd && editor.document.fileName.endsWith('.md')) {
                    ...
                    vscode.window.showWarningMessage('Zef: Only works in .zef.md, .py, or .rs files');
                }
                return;
            }
        ...
        vscode.commands.registerCommand('zef.openPreview', () => {
            ...
            if (!editor || !isZefDocument(editor.document)) {
                ...
                if (!treatAllMd && editor?.document.fileName.endsWith('.md')) {
                    ...
                    vscode.window.showWarningMessage('Zef: Open a .zef.md, .py, or .rs file first');
                }
                return;
            }
